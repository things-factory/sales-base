          ...
          .map(async (orderInv: OrderInventory) => {
            if (orderInv?.inventory?.id) {
              ...
              return {
                ...
                releaseStdUnitValue: orderInv.releaseStdUnitValue,
                status: orderInv.status
              }
            } else {
              const { batchId, product, packingType, releaseQty, releaseStdUnitValue } = orderInv
...
async function getAvailableAmount(
  ...
  const result: any[] = await getRepository(Inventory).query(`
    ...
    SELECT
      ...
      SUM(COALESCE(i.std_unit_value, 0)) - SUM(COALESCE(i.locked_std_unit_value, 0)) - MAX(COALESCE(oi.release_std_unit_value, 0)) as "std_unit_value"
    FROM
      inventories i
  ...
  if (result?.length) {
    qty = result[0].qty
    stdUnitValue = result[0].std_unit_value
  }
